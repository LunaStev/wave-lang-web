...
```wave
extern(c) функция имя_функции(аргументы...) -> возвратный_тип;
```
...
```wave
extern(c) функция printf(fmt: указатель<u8>);
```
...
```wave
extern(c) fun InitWindow(width: i32, height: i32, title: указатель<u8>);
```
...
```wave
extern(c) {
    fun InitWindow(width: i32, height: i32, title: указатель<u8>);
    fun CloseWindow();
    fun BeginDrawing();
    fun EndDrawing();
}
...
```wave
extern(c) {
    fun my_puts(указатель<i8>) "puts";
    fun my_strlen(указатель<i8>) "strlen";
}
```
