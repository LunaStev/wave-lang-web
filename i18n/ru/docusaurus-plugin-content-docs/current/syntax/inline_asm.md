...
Инлайн-ассемблер Wave создается с помощью блока `asm { ... `}. 이 기능은 운영체제, UEFI 부트로더, 시스템 호출, 포트 I/O, CPU 제어처럼 일반 Wave 문법만으로 표현하기 어려운 저수준 코드를 위해 존재합니다.
...
asm {
    "инструкция"
    in("ограничение_или_рег") значение
    out("ограничение_или_рег") цель
    clobber("элемент")
}
```
