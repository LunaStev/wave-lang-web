...
Цель вывода(`out(...) `target\`) в текущей реализации рекомендуется следующий шаблон.

- Переменная: `out("rax") ret`
- Разыменование указателя: `out("rax") deref p`
...
- Специальные: `"memory"`, `"cc"` (внутренняя нормализация по целям)б
