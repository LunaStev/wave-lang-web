...
fun main() {
    ...
    if (n >= 0) {
        // buf - строка, завершающаяся на NUL
    } else if (n == ENV_ERR_NOT_FOUND) {
        // ключ не найден
    } else if (n == ENV_ERR_NO_SPACE) {
        // не хватает места в буфере
    }
}
```

## 2. Чтение целочисленной переменной окружения (обобщенный тип результата)
...
## 3. Текущая директория/перемещение
