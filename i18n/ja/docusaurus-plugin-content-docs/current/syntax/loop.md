...
Waveのfor初期化は複数の形式をサポートしています。

- 暗黙の`var`タイプ初期化
- `var` / `let mut` / `const` 宣言初期化
- 一般式初期化（既存変数再利用）
...
宣言型初期化（`var`、`let mut`、`i :i32 = ...`）はループスコープ変数として動作し、式に基づく初期化（`i = ...`）は外側の変数そのものを更新します。
...
これにより、(`while`, `while`) 形式の組み合わせを順次処理できます。
