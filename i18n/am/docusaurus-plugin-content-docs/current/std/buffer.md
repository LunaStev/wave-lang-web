...
`std::buffer` ተለዋዋጭ ባይት ቋት እና አጠቃላይ ማቋቋሚያዎችን ያቀርባል።
...
## 2) አጠቃላይ ቋት (`TypedBuffer<T>`)

Wave ምንም አይነት ማጣቀሻ የለውም፣ ስለዚህ አይነት ነጋሪ እሴቶች ተገልጸዋል።
...
- `elem_size` በጠዋዩ በትክክል መግባት አለበት።
- ከክልል ውጪ መዳረሻ `false` ወይም ነባሪው (`buffer_at` `0` ነው) ይመልሳል።
