...
در export بلوکی، نام هر تابع را به‌عنوان نماد عمومی استفاده می‌کند. `export(c, "symbol") { ... }` مجاز نیست، چون یک alias مشترک برای چند تابع باعث برخورد نمادها می‌شود.
